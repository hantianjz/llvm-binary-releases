...
class LLVMBinaryProcessor:
    ...
    def extract_metadata(self) -> Dict:
        ...
        if "ARM64" in url_upper or "aarch64" in url_upper:
            platform = "arm64"
        ...
        if "macos" in url_lower or "apple" in url_lower or "darwin" in url_lower:
            os_name = "darwin"
        elif "linux" in url_lower:
            os_name = "linux"
        elif "windows" in url_lower or "win64" in url_lower or "win" in url_lower:
            os_name = "windows"
        else:
            os_name = "unknown"
    ...
    def matches_binary_filter(self, path: Path) -> bool:
        ...
        return (
            name in self.binary_filter
            or (name.endswith(".exe") and name[:-4] in self.binary_filter)
            or (not name.endswith(".exe") and f"{name}.exe" in self.binary_filter)
        )
    ...
    def process(self):
        ...
        try:
            ...
            metadata["processed_binaries"] = sorted(
                [b.name[:-4] if b.name.lower().endswith(".exe") else b.name for b in binaries]
            )
...
def main():
    ...
    parser.add_argument(
        "-e", "--extract-dir", type=Path, default=Path("./extract-cache"), help="Directory for extracted files cache"
    )
    parser.add_argument(
        "-f",
        "--binaries-file",
        type=Path,
        default=Path("./binaries.txt"),
        help="File containing list of binaries to extract (default: ./binaries.txt)",
    )
    parser.add_argument(
        "--list-binaries", action="store_true", help="List all available binaries in the tarball and exit"
    )
    parser.add_argument("--clean-cache", action="store_true", help="Clean the cache directories and exit")
    parser.add_argument("--no-cache", action="store_true", help="Disable caching, always download fresh")
