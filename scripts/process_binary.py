...
class LLVMBinaryProcessor:
    ...
    def extract_metadata(self) -> Dict:
        ...
        filename = Path(self.binary_url).name
        
        # Extract version - look for semantic version between dashes
        version = "unknown"
        version_pattern = r'-(\d+\.\d+\.\d+)-'  # matches -X.Y.Z- pattern
        match = re.search(version_pattern, filename)
        if not match:
            # Try without trailing dash (end of string)
        ...
        if "ARM64" in url_upper or "AARCH64" in url_upper:
            platform = "arm64"
        elif any(x in url_upper for x in ["X86_64", "X64", "AMD64"]):
            platform = "x86_64"
        else:
            platform = "unknown"
        ...
        if any(x in url_lower for x in ["macos", "darwin", "apple"]):
            os_name = "darwin"
        elif "linux" in url_lower:
            os_name = "linux"
        elif any(x in url_lower for x in ["windows", "win64", "win32"]):
            os_name = "windows"
        else:
            os_name = "unknown"
